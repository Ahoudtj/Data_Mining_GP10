...
# Group Good and Start Credit to be "Acceptable" and leave bad as "Unacceptable"
table(TRAIN$Credit_Score)
TRAIN$Credit_Score <- as.factor(ifelse(TRAIN$Credit_Score == "Poor", 0, 1))
...
# Only remove for the predictive model then add back
TRAIN$ID <- NULL
...
```{r Decision Tree}
TREE <- rpart(Credit_Score~.,data=TRAIN) # Create Decision Tree Model
...
RPART <- train(Credit_Score~.,
               ...
               preProc=c("center","scale"),
               method = "rpart",
               tuneGrid=rpartGrid) # Create RPART model
